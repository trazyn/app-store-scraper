...
function metadata(url) {
  return new Promise(function (resolve) {
    ...
    resolve(common.request(url, {}, { responseType: "text" }));
  }).then((html) => {
    if (html.length === 0) {
...
function transformArtwork(artwork) {
  ...
  const width =
    artwork.width ||
    (artwork.variants && artwork.variants[0] && artwork.variants[0].width) ||
    1920;
  const height =
    artwork.height ||
    (artwork.variants && artwork.variants[0] && artwork.variants[0].height) ||
    1080;

  // background/text colors
  const bgHex =
    rgbObjToHex(artwork.backgroundColor) ||
    rgbObjToHex(artwork.bgColor) ||
    "4b5782";
  const textHex =
    rgbObjToHex(artwork.textColor) ||
    rgbObjToHex(artwork.textColour) ||
    "f2ebe3";
...
function parseEvents(html) {
  ...
  const jsonStr = ele.html();
  if (!jsonStr) {
    throw new Error("App page missing serialized-server-data");
  }
  const data = JSON.parse(jsonStr);
  // Support both shapes: { data: [ { data: rawData } ] } (current) and [ { data: rawData } ] (legacy)
  ...
  const appEvents = rawData.shelfMapping?.appEvents?.items || [];
  ...
  if (
    videoPreviewsByType &&
    videoPreviewsByType["iphone6+"] &&
    videoPreviewsByType["iphone6+"].length > 0
  ) {
